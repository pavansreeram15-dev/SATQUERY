...
def classify_intent(prompt: str, persona: UserPersona = UserPersona.PUBLIC_RESEARCHER) -> Tuple[QueryIntent, float]:
    ...
    # 6. Maritime, Live AIS Vessel & Submarine Cable queries
    if re.search(r'\b(vessel|vessels|ais|ship tracking|maritime|ship mmsi|mmsi|track this vessel|vessels matching|vessels near|cargo ship|cargo ships|tanker|tankers|suez canal|panama canal|port of|cable|cables|submarine cable|landing point|fiber optic)\b', text):
